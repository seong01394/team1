...
public class CommuProc implements CommuProcInter {
  ...
  @Autowired
  private CommuDAOInter commuDAO;
  ...
  public int create(CommuVO commuVO) {
    int cnt = this.commuDAO.create(commuVO);
  ...
  public ArrayList<CommuVO> list_all() {
    ArrayList<CommuVO> list = this.commuDAO.list_all();
  ...
  public ArrayList<CommuVO> list_by_clubno(int clubno) {
    ArrayList<CommuVO> list = this.commuDAO.list_by_clubno(clubno);
    return list;
  }
  ...
  public CommuVO read(int communo) {
    CommuVO commuVO = this.commuDAO.read(communo);
    return commuVO;
  }

  @Override
  public int youtube(HashMap<String, Object> map) {
    int cnt = this.commuDAO.youtube(map);
    return cnt;
  }

  @Override
  public ArrayList<CommuVO> list_by_clubno_search(HashMap<String, Object> hashMap) {
    ArrayList<CommuVO> list = this.commuDAO.list_by_clubno_search(hashMap);
    return list;
  }
  
  @Override
  public int list_by_clubno_search_count(HashMap<String, Object> hashMap) {
    int cnt = this.commuDAO.list_by_clubno_search_count(hashMap);
    
    return cnt;
  };

  @Override
  public ArrayList<CommuVO> list_by_clubno_search_paging(HashMap<String, Object> map) {
    /*
     ...
     */
    int begin_of_page = ((int)map.get("now_page") - 1) * Commu.RECORD_PER_PAGE;
    ...
    // 2 페이지 = 10 + 10: 20
    // 3 페이지 = 20 + 10: 30
    int end_num = begin_of_page + Commu.RECORD_PER_PAGE;
    /*
    ...
    ArrayList<CommuVO> list = this.commuDAO.list_by_clubno_search_paging(map);
    
    return list;
  }

  @Override
  public String pagingBox(int clubno, int now_page, String hashtage, String list_file, int search_count, int record_per_page,
      int page_per_block) {
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    if (now_grp >= 2) { // 현재 그룹번호가 2이상이면 페이지수가 11페이지 이상임으로 이전 그룹으로 갈수 있는 링크 생성
      str.append("<span class='span_box_1'><A href='" + list_file + "?clubno="+clubno+"&hashtage=" + hashtage + "&now_page=" + _now_page
          + "'>이전</A></span>");
    }
    ...
    for (int i = start_page; i <= end_page; i++) {
      ...
      if (now_page == i) { // 목록에 출력하는 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
        ...
        str.append("<span class='span_box_1'><A href='" + list_file + "?clubno="+clubno+"&hashtage=" + hashtage + "&now_page=" + i + "'>" + i
            + "</A></span>");
      }
    }
  ...
  public int update_text(CommuVO commuVO) {
   int cnt = this.commuDAO.update_text(commuVO);
  ...
  public int update_file(CommuVO commuVO) {
    int cnt = this.commuDAO.update_file(commuVO);
  ...
  public int delete(int communo) {
   int cnt = this.commuDAO.delete(communo);
  ...
  public int count_by_clubno(int clubno) {
    int cnt = this.commuDAO.count_by_clubno(clubno);
  ...
  public int delete_by_clubno(int clubno) {
    int cnt = this.commuDAO.delete_by_clubno(clubno);
  ...
  public int count_by_memberno(int memberno) {
    int cnt = this.commuDAO.count_by_memberno(memberno);
  ...
  public int delete_by_memberno(int memberno) {
    int cnt = this.commuDAO.delete_by_memberno(memberno);
